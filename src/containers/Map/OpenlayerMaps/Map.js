...
class MapComponent extends Component {
  ...
  showNotifications = () => {
    ...
    if (redAircraftCount > 0) {
      const redNotification = "Ada potensi tabrakan diatas 80%";
      this.showNotification(notificationsContainer, redNotification, 'red');
    } else if (blueAircraftCount > 0) {
      const blueNotification = "Ada potensi tabrakan diatas 20%";
...
checkAcasPairsInFlights = () => {
  ...
  addAircraftsToMap(aircrafts) {
    ...
    aircrafts.forEach((aircraft) => {
      ...
      if (lon && lat) {
        ...
        const marker = new Overlay({
          ...
          element: createMarkerElement(icao, heading, pra, color), // Mengirimkan warna dan PRA sebagai argumen
        });
    ...
    function createMarkerElement(icao, heading, pra, color) {
      ...
      if (pra !== null) {
        const praText = document.createElement('div');
        praText.className = 'pra-text';
        praText.innerHTML = `PRA: ${(pra * 100).toFixed(2)}%`;
        praText.style.color = color; // Mengatur warna teks PRA
        markerElement.appendChild(praText);
      }
  ...
  toggleIcaoVisibility = () => {
    ...
    const praTextElements = document.getElementsByClassName('pra-text');
    ...
    for (let i = 0; i < praTextElements.length; i++) {
      const praTextElement = praTextElements[i];
      praTextElement.style.display = praTextElement.style.display === 'none' ? 'block' : 'none';
    }
  };
